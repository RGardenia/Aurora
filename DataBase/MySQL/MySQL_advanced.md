...
![image-20220428193416981](../../doc/images/image-20220428193416981.png)
...
三个关键特性：插入缓冲（insertbuffer）、二次写（double write）、自适应哈希索引（adaptive hash index）

## insert/change [buffer](https://so.csdn.net/so/search?q=buffer&spm=1001.2101.3001.7020)
...
**<font size=5>当出现change buffer时，更新一条内存中不存在的页(和flush链表的区别)</font>**
...
**<font size=5>change buffer中存在数据时发生宕机怎么办？</font>**

每次change buffer中的数据会同步到redo log中，数据库异常崩溃，能够从redo log中恢复数据。

**<font size=5>为什么change buffer是只针对于二级索引的优化呢？</font>**
...
![image-20220527122317719](../images/image-20220527122317719.png)
...
做完上面这些，事务就可以完成了。所以，你会看到，执行这条更新语句的成本很低，就是写了两处内存，然后**写了一处磁盘（两次操作合在一起写了一次磁盘），****而且还是顺序写的**。（注意：上述的三个步骤是一个事务，也就是必须redo log写完，事务才算搞定，这也印证了为啥redo log一定可以恢复change buffer中的数据）
...
![image-20220527122349877](../images/image-20220527122349877.png)
...
我们知道InnoDB是不支持[Hash](https://so.csdn.net/so/search?q=Hash&spm=1001.2101.3001.7020)索引的。最大的原因是因为这个数据结构不支持范围查询，在MySQL的使用环境来说，这个是非常不友好的。
