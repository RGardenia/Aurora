...
![编译和解释型语言区别](D:\Download\IT 自学笔记\计算机\shell分享\pictures/编译和解释型语言区别.png)
...
![语言分类](D:\Download\IT 自学笔记\计算机\shell 编程\pictures/语言分类.png)

- **总结**

编译型语言比解释型语言**速度较快**，但是不如解释型语言跨平台性好。如果做底层开发或者大型应用程序或者操作系开发一般都用编译型语言；如果是一些服务器脚本及一些辅助的接口，对速度要求不高、对各个平台的兼容性有要求的话则一般都用解释型语言。

## 2. shell 简介

![00_shell介绍](D:\Download\IT 自学笔记\计算机\shell 编程\pictures/00_shell介绍.png)
...
![01_shell介绍](D:\Download\IT 自学笔记\计算机\shell 编程\pictures/01_shell介绍.png)
...
```powershell
1) 编写人生第一个shell脚本
[root@zy shell01]# cat first_shell.sh
#!/bin/env bash
...
2) 脚本增加可执行权限
[root@zy shell01]# chmod +x first_shell.sh

3) 标准方式执行脚本
[root@zy shell01]# pwd
/shell01
[root@zy shell01]# /shell01/first_shell.sh
或者
[root@zy shell01]# ./first_shell.sh
...
```powershell
[root@zy shell01]# bash first_shell.sh
[root@zy shell01]# sh first_shell.sh
[root@zy shell01]# bash -x first_shell.sh
+ echo 'hello world'
hello world
+ echo 'hello world'
hello world
...
```powershell
[root@zy shell01]# source first_shell.sh
hello world
hello world
hello world
```
...
```powershell
[root@zy ~]# A=hello			定义变量A
[root@zy ~]# echo $A			调用变量A，要给钱的，不是人民币是美元"$"
hello
[root@zy ~]# echo ${A}		还可以这样调用，不管你的姿势多优雅，总之要给钱
hello
[root@zy ~]# A=world			因为是变量所以可以变，移情别恋是常事
[root@zy ~]# echo $A			不管你是谁，只要调用就要给钱
world
[root@zy ~]# unset A			不跟你玩了，取消变量
[root@zy ~]# echo $A			从此，我单身了，你可以给我介绍任何人
...
[root@zy ~]# echo $A
1234567
[root@zy ~]# echo ${A:2:4}		表示从A变量中第3个字符开始截取，截取4个字符
3456
...
[root@zy ~]# echo $name
harry
[root@zy ~]# read -p "Input your name:" name
Input your name:tom
[root@zy ~]# echo $name
tom
```
...
~~~powershell
[root@zy ~]# export A=hello		临时将一个本地变量（临时变量）变成环境变量
[root@zy ~]# env|grep ^A
A=hello
...
[root@zy ~]# unset i j
[root@zy ~]# i=1;j=1
[root@zy ~]# let x=i++         先赋值，再运算
[root@zy ~]# let y=++j         先运算，再赋值
[root@zy ~]# echo $i
2
[root@zy ~]# echo $j
2
...
```powershell
[root@zy shell05]# declare -A books
[root@zy shell05]# let books[linux]++
[root@zy shell05]# declare -A|grep books
