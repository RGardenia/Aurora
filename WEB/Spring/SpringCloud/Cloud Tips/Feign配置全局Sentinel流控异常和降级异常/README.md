...
最近在使用feign调用的时候，因为加入了sentinel进行流控和降级机制，在如果没有对Sentinel异常处理进行封装的话，默认返回的信息如下所示

```bash
Blocked by Sentinel(flow limiting)
```

很显然上述的异常信息，对目前的项目来说不太适用，所以就打算在对异常进行封装，使其返回项目中统一的格式

## Feign全局兜底异常处理

之前提到过通过 [配置Sentinel规则持久化到Nacos中](http://www.moguit.cn/#/info?blogUid=6473a200c9986f45af2ae9f12534ec93)
提到将Sentinel规则持久化到数据库中，但是在通过feign调用时，不能很好的处理由sentinel产生的流控异常，当然可以通过 

**@SentinelResource** 注解来解决，如下所示，通过在需要处理的方法上，编写一个异常处理的方法即可
...
但是这个解决的方法比较麻烦，因为如果的接口很多的话，那么需要在每个需要处理的接口上都进行添加，因此可以考虑定义一个全局的兜底异常处理。

需要做的就是在feign上在编写一套全局的异常处理，首先有一个已经写好的feign接口PictureFeignClient.Java，在下面的
@Feign注解上，需要添加全局fallback类
...
然后需要在编写一个全局fallback类：PictureFeignFallback.java，在这里类中，就会对feign调用服务有一个兜底的方法，当的Picture服务宕机的时候，导致接口不可用时，就会自动触发该方法
...
同时，使用了自定义的返回方式，对的错误返回进行了封装，达到了全局兜底异常的处理

还需要修改一下配置，在的yml文件中 激活Sentinel对Feign的支持
...
上面提到的异常指的就是当服务不可用或者服务出现业务异常时触发的，也就是满足的降级规则时触发的全局兜底异常。但是除了降级异常之外，还有通过配置的规则，设置的流控规则

![image-20201005103930483](images/image-20201005103930483.png)

如上图所示，就是对某个接口配置的流控规则，其中流控规则主要分为一下几种
...
所以针对如上所示的各种异常，也需要进行全局流控异常的捕获和处理，首先创建 全局feign异常处理
